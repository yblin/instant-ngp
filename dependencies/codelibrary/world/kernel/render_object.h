...
public:
    ...
    /**
     * Add a line into this object.
     */
    void AddLine(const FPoint3D& p1, const FPoint3D& p2, const RGB32Color& c) {
        RenderData data(GL_LINES);
        data.vertices = {p1, p2};
        data.colors = {c, c};
        data.indices = {0, 1};
        this->Add(data);
    ...
    /**
     * Add a line loop of the given points.
     */
    void AddLine(const Array<FPoint3D>& points, const RGB32Color& c) {
        if (points.size() <= 1) return;

        RenderData data(GL_LINES);
        data.vertices = points;
        data.colors.assign(points.size(), c);
        data.indices.resize(points.size());
    ...
    /**
     * Add the box's contour.
     */
    void AddLine(const FBox3D& box, const RGB32Color& c) {
        RenderData data(GL_LINES);
        data.vertices = {
     ...
     * Add a point set.
     */
    void AddPoint(const Array<FPoint3D>& points,
                  const Array<RGB32Color>& colors) {
        CHECK(data_.empty() || data_.type == GL_POINTS);
        CHECK(points.size() == colors.size());
        if (points.empty()) return;

        data_.type = GL_POINTS;
        modified_ = true;
        bounding_box_.Join(FBox3D(points.begin(), points.end()));
        int n1 = data_.vertices.size();
        int n2 = points.size();
        data_.vertices.insert(points);
        data_.indices.reserve(n1 + n2);
        CHECK(data_.indices.size() == n1);
        for (int i = 0; i < n2; ++i) {
            data_.indices.push_back(n1 + i);
        }
        if (data_.colors.empty()) {
            data_.colors.assign(n1, default_color_);
        }
        data_.colors.insert(colors);

        if (!data_.normals.empty()) {
            data_.normals.resize(n1 + n2, default_normal_);
        }
        if (!data_.texture_coords.empty()) {
            data_.texture_coords.resize(n1 + n2, FPoint2D(0.0f, 0.0f));
        }
    }
     ...
     * Add a point set.
     */
    void Add(const Array<FPoint3D>& points) {
        CHECK(data_.empty() || data_.type == GL_POINTS);
        if (points.empty()) return;
        ...
        if (!data_.colors.empty()) {
            data_.colors.resize(n1 + n2, default_color_);
        }
        if (!data_.normals.empty()) {
            data_.normals.resize(n1 + n2,  default_normal_);
        }
